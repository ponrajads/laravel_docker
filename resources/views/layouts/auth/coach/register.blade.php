...
<script>
    ...
    function validateStep1() {
        ...
        if (!fathername) {
            $("#father_name-error").html("Father Name is required.");
            return false;
        }else{
...
if (aadhar.length !== 12) {
    ...
    // Add more specific Aadhar card number validation if needed
    ...
    if (!address1) {
        $("#address1-error").html("Address Line 1 is required.");
        return false;
    } else {
        $("#address1-error").html("");
    }
    ...
    function validateStep4() {
        ...
        const qualification = document.querySelector('select[name="official_type"]').value.trim();
    if (!qualification) {
        $("#qualification_exam-error").html("Official Type is required.");
        return false;
    } else {
        $("#qualification_exam-error").html("");
    }

    const year_of_passing = document.querySelector('input[name="date_of_certificate"]').value.trim();
    if (!year_of_passing) {
        $("#year_of_passing-error").html("Certification Passed is required.");
        return false;
    } else {
        $("#year_of_passing-error").html("");
    }
    ...
    // Validate DOB Proof
    const dobProof = document.querySelector('input[name="dob_proof"]').value.trim();
    if (!dobProof) {
    ...
    var permanentAddressFields = ['address1', 'address2', 'pincode', 'state', 'district', 'city'];
    var communicationAddressFields = ['caddress1', 'caddress2', 'cpincode', 'cstate', 'cdistrict', 'ccity'];

    // Function to autofill communication address based on permanent address
    function autofillCommunicationAddress() {
        // Check if the checkbox is checked
        var sameAddressCheckbox = document.getElementById('sameAddressCheckbox');
        if (sameAddressCheckbox.checked) {
            // Loop through the permanent address fields and copy values to communication address fields
            for (var i = 0; i < permanentAddressFields.length; i++) {
                var permanentField = document.getElementsByName(permanentAddressFields[i])[0];
                var communicationField = document.getElementsByName(communicationAddressFields[i])[0];

                // Handle special case for Tom Select dropdown (state)
                if (permanentAddressFields[i] === 'state') {
                    var permanentStateSelect = document.querySelector('[name="state"]');
                    var communicationStateSelect = document.querySelector('[name="cstate"]');

                    // Set the selected value for Tom Select dropdown
                    var selectedValue = permanentStateSelect.options[permanentStateSelect.selectedIndex].value;
                    communicationStateSelect.tomselect.addOption({ value: selectedValue, text: selectedValue });
                    communicationStateSelect.tomselect.setValue([selectedValue]);
                } else {
                    // Copy values for non-dropdown fields
                    communicationField.value = permanentField.value;
                }
            }
        } else {
            // Clear communication address fields if the checkbox is unchecked
            for (var i = 0; i < communicationAddressFields.length; i++) {
                var communicationField = document.getElementsByName(communicationAddressFields[i])[0];

                // Handle special case for Tom Select dropdown (state)
                if (communicationAddressFields[i] === 'cstate') {
                    var communicationStateSelect = document.querySelector('[name="cstate"]');

                    // Clear the selected value for Tom Select dropdown
                    communicationStateSelect.tomselect.clear();
                } else {
                    // Clear values for non-dropdown fields
    ...
    // Attach the autofill function to the checkbox change event
    sameAddressCheckbox.addEventListener('change', autofillCommunicationAddress);
});

function toggleOtherQualificationInput() {
        var qualificationSelect = document.getElementById('official_type');
