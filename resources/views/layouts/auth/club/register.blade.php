<!DOCTYPE html>
<html lang="en" class="light">
<meta http-equiv="content-type" content="text/html;charset=utf-8" />
<head>
        <meta charset="utf-8">
        <link href="{{asset('dist/images/logo/rugbylogo.png')}}" rel="shortcut icon">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta name="description" content="Rugby India, founded in 1998, is the sole governing body for the sport of Rugby in India. Recognized by the Ministry of Youth Affairs & Sport, Govt. of India, Rugby India is a full member of World Rugby, Asia Rugby and the Indian Olympic Association (IOA). The body is responsible for the growth & development of the sport of Rugby across the country from the grassroots to the high performance level.">
        <meta name="keywords" content="Indian Rugby">
        <meta name="author" content="Sportingindia">
        <title>Indian Rugby Football Union</title>
        <!-- BEGIN: CSS Assets-->
        <link href="https://fonts.googleapis.com/css2?family=Alumni+Sans:ital,wght@0,400;0,500;0,700;0,800;0,900;1,400;1,500;1,600;1,700;1,800;1,900&family=Fjalla+One&family=Oswald:wght@200;300;400;500;600;700&display=swap" rel="stylesheet">
        <link rel="stylesheet" href="{{asset('dist/css/app.css')}}"/>

        <!-- END: CSS Assets-->
		<style>
		[x-cloak] {
			display: none;
		}

		[type="checkbox"] {
			box-sizing: border-box;
			padding: 0;
		}

		.form-checkbox,
		.form-radio {
			-webkit-appearance: none;
			-moz-appearance: none;
			appearance: none;
			-webkit-print-color-adjust: exact;
			color-adjust: exact;
			display: inline-block;
			vertical-align: middle;
			background-origin: border-box;
			-webkit-user-select: none;
			-moz-user-select: none;
			-ms-user-select: none;
			user-select: none;
			flex-shrink: 0;
			color: currentColor;
			background-color: #fff;
			border-color: #e2e8f0;
			border-width: 1px;
			height: 1.4em;
			width: 1.4em;
		}

		.form-checkbox {
			border-radius: 0.25rem;
		}

		.form-radio {
			border-radius: 50%;
		}

		.form-checkbox:checked {
			background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3cpath d='M5.707 7.293a1 1 0 0 0-1.414 1.414l2 2a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0-1.414-1.414L7 8.586 5.707 7.293z'/%3e%3c/svg%3e");
			border-color: transparent;
			background-color: currentColor;
			background-size: 100% 100%;
			background-position: center;
			background-repeat: no-repeat;
		}

		.form-radio:checked {
			background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3ccircle cx='8' cy='8' r='3'/%3e%3c/svg%3e");
			border-color: transparent;
			background-color: currentColor;
			background-size: 100% 100%;
			background-position: center;
			background-repeat: no-repeat;
		}
	</style>
	<style>
        /* Add some basic styling for the preview image */
        #imagePreview {
            width: 150px;
            height: 150px;
            border-radius: 50%;
            object-fit: cover;
        }
    </style>
<body class="login" id="registrationpage">

@yield('content')
     <!-- BEGIN: JS Assets-->
     <script src="{{asset('dist/js/app.js')}}"></script>
        <!-- END: JS Assets-->

		<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/js/bootstrap.bundle.min.js"></script>
 <!-- Load jQuery first -->
<script src="https://code.jquery.com/jquery-3.6.4.min.js"></script>

<!-- Then load Alpine.js -->
<script src="https://cdn.jsdelivr.net/npm/alpinejs@2.8.2/dist/alpine.min.js" defer></script>

	<script>
		function app() {
			return {
				step: 1,
				passwordStrengthText: '',
				togglePassword: false,

				image: 'data:image/jpeg;base64,',
                image1: 'data:image/jpeg;base64,',
                image2: 'data:image/jpeg;base64,',
				password: '',
				gender: 'Male',

				checkPasswordStrength() {
					var strongRegex = new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})");
					var mediumRegex = new RegExp("^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})");

					let value = this.password;

					if (strongRegex.test(value)) {
						this.passwordStrengthText = "Strong password";
					} else if(mediumRegex.test(value)) {
						this.passwordStrengthText = "Could be stronger";
					} else {
						this.passwordStrengthText = "Too weak";
					}
				}
			}
		}
	</script>
	<!-- Add a script tag to include your JavaScript code -->
<script>
    // Initialize the current step
    function ValidateAlpha(evt)
    {

      var ASCIICode = (evt.which) ? evt.which : evt.keyCode
        if (ASCIICode > 31 && (ASCIICode < 65 || ASCIICode > 90) && (ASCIICode < 97 || ASCIICode > 122) && ASCIICode != 32 )
            return false;
        return true;


    }
	function onlyNumberKey(evt) {

		  // Only ASCII charactar in that range allowed
		  var ASCIICode = (evt.which) ? evt.which : evt.keyCode
		  if (ASCIICode > 31 && (ASCIICode < 48 || ASCIICode > 57))
			  return false;
		  return true;
	  }
	  function ValidateAlphaNumaric(evt)
    {

      var ASCIICode = (evt.which) ? evt.which : evt.keyCode
        if (ASCIICode > 31 && (ASCIICode < 65 || ASCIICode > 90) && (ASCIICode < 48 || ASCIICode > 57) && ASCIICode != 32)
            return false;
        return true;


    }

    var baseUrl = {!! json_encode(url('/')) !!}
	function checkEmailExist(){
        var id = document.getElementById("email").value;
        $.ajax({
            url: baseUrl+"/getEmailCheck/" + id,
            method: "GET",
            success: function(response){
                if(response.check == "1" || response.check == 1){
                    //alert("Email Already Exist");
                    document.getElementById("email-error").innerHTML = id+" already exist";
                    document.getElementById("email").value='';
                    $("#email").css('border-color','red');
                }else{
                    document.getElementById("email-error").innerHTML = "";
                    $("#email").css('border-color','');
                }
            }
        });
    }
	function checkMobileExist(){
        var id = document.getElementById("mobile").value;
        $.ajax({
            url: baseUrl+"/getMobileCheck/" + id,
            method: "GET",
            success: function(response){
                if(response.check == "1" || response.check == 1){
                    //alert("Email Already Exist");
                    document.getElementById("mobile-error").innerHTML = id+" already exist";
                    document.getElementById("mobile").value='';
                    $("#mobile").css('border-color','red');
                }else{
                    document.getElementById("mobile-error").innerHTML = "";
                    $("#mobile").css('border-color','');
                }
            }
        });
    }

    function validateAndNext() {
        // Add your validation logic here
        if (!validateStep1()) {

            return;
        }

        if (!validateStep2()) {

            return;
        }

		if (!validateStep3()) {

			return;
		}


        // Add similar checks for other steps

        // If all validation passes, proceed to the next step

        //updateStepDisplay();
    }

    function validateStep1() {
		document.getElementById('steponestep').style.display = 'block';
		$('#stepone').css("display","block");
		$('#steptwo').css("display", "none");
		$('#stepthree').css("display", "none");


        // Validation logic for step 1
        // Return true if validation passes, false otherwise
        const acadmey_name = document.querySelector('input[name="acadmey_name"]').value.trim();
        if (!acadmey_name) {
            $("#acadmey_name-error").html("Acadmey Name is required.");
            return false;
        }else{
			$("#acadmey_name-error").html("");
		}
        const association = document.querySelector('select[name="association"]').value.trim();
    if (!association) {
        $("#association-error").html("Name of the Member Unit is required.");
        return false;
    } else {
        $("#association-error").html("");
    }

		const email = document.getElementById('email').value.trim();

    if (!email) {
        $("#email-error").html("Email fields are required.");
        return false;
    }else{

    $("#email-error").html("");
    }
	const mobile = document.getElementById('mobile').value.trim();


    if (!mobile) {
        $("#mobile-error").html("Mobile fields are required.");
        return false;
    }else{

    $("#mobile-error").html("");
    }

    // Add more specific Aadhar card number validation if needed

        // Add more validation checks as needed

        return true;
    }

    function validateStep2() {
		document.getElementById('steponestep').style.display = 'none';
		$('#stepone').css("display","none");
		$('#steptwo').css("display", "block");
		$('#stepthree').css("display", "none");


        // Validation logic for step 2
        // Return true if validation passes, false otherwise
        // Validate Father / Guardian Name
        const caddress1 = document.querySelector('input[name="caddress1"]').value.trim();
    if (!caddress1) {
        $("#address1-error").html("Address Line 1 is required.");
        return false;
    } else {
        $("#address1-error").html("");
    }


    // Validate Pincode
    const cpincode = document.querySelector('input[name="cpincode"]').value.trim();
    if (!cpincode) {
        $("#pincode-error").html("Pincode is required.");
        return false;
    } else {
        $("#pincode-error").html("");
    }

    const cstate = document.querySelector('select[name="cstate"]').value.trim();
    if (!cstate) {
        $("#state-error").html("State is required.");
        return false;
    } else {
        $("#state-error").html("");
    }

    const cdistrict = document.querySelector('input[name="cdistrict"]').value.trim();
    if (!cdistrict) {
        $("#district-error").html("District is required.");
        return false;
    } else {
        $("#district-error").html("");
    }

    const ccity = document.querySelector('input[name="ccity"]').value.trim();
    if (!ccity) {
        $("#city-error").html("City is required.");
        return false;
    } else {
        $("#city-error").html("");
    }


    const club_director = document.querySelector('input[name="club_director"]').value.trim();
    if (!club_director) {
        $("#club-error").html("Name of the Member Unit is required.");
        return false;
    } else {
        $("#club-error").html("");
    }



		// Return true if validation passes, false otherwise
		return true;
	}
	function validateStep3() {
		document.getElementById('steponestep').style.display = 'none';
		$('#stepone').css("display","none");
		$('#steptwo').css("display", "none");
		$('#stepthree').css("display", "block");


		const photograph = document.querySelector('input[name="photo"]').value.trim();
    if (!photograph) {
        $("#photograph-error").html("Photograph is required.");
        return false;
    }else{
		$("#photograph-error").html("");
	}
    const aadhar = document.querySelector('input[name="academey_aadhar"]').value.trim();
    if (!aadhar) {
        $("#aadhar-proof-error").html("Head Aadhar Card is required.");
        return false;
    }else{
		$("#aadhar-proof-error").html("");
	}
    // Validate DOB Proof
    const academeyphoto = document.querySelector('input[name="academey_photo"]').value.trim();
    if (!academeyphoto) {
        $("#academy-photo-error").html("Academy Photo is required.");
        return false;
    }else{
		$("#academy-photo-error").html("");
	}


    // Validate Address Proof
    const academeyLogo = document.querySelector('input[name="academey_logo"]').value.trim();
    if (!academeyLogo) {
        $("#academy-logo-error").html("Academy Logo is required.");
        return false;
    }else{
		$("#academy-logo-error").html("");
	}

    // Return true if validation passes, false otherwise
    return true;
	}


    // Validate Photograph

    function updateStepDisplay() {
        // Hide/show elements based on the current step
        document.querySelectorAll('[data-step]').forEach(element => {
            const elementStep = parseInt(element.getAttribute('data-step'));
            element.style.display = elementStep === step ? 'block' : 'none';
        });
    }


    // Get references to the elements
</script>
<script>
        // JavaScript function to preview the selected image
        function previewImage(input) {
            const imagePreview = document.getElementById('imagePreview');
            const file = input.files[0];

            if (file) {
                const reader = new FileReader();

                reader.onload = function(e) {
                    // Set the source of the image preview to the selected image
                    imagePreview.src = e.target.result;
                };

                reader.readAsDataURL(file);
            }
        }
    </script>
	<script>
    // JavaScript function to preview the selected image for DOB Proof
    function previewDOBImage(input) {
        const dobImagePreview = document.getElementById('dobImagePreview');
        const file = input.files[0];

        if (file) {
            const reader = new FileReader();

            reader.onload = function(e) {
                // Set the source of the DOB Image preview to the selected image
                dobImagePreview.src = e.target.result;
            };

            reader.readAsDataURL(file);
        }
    }
</script>
<script>
    // JavaScript function to preview the selected image for Address Proof
    function previewAddressProofImage(input) {
        const addressProofImagePreview = document.getElementById('addressProofImagePreview');
        const file = input.files[0];

        if (file) {
            const reader = new FileReader();

            reader.onload = function(e) {
                // Set the source of the Address Proof Image preview to the selected image
                addressProofImagePreview.src = e.target.result;
            };

            reader.readAsDataURL(file);
        }
    }
</script>
<script>
    function previewaadharImage(input) {
        const dobImagePreview = document.getElementById('aadharImagePreview');
        const file = input.files[0];

        if (file) {
            const reader = new FileReader();

            reader.onload = function(e) {
                // Set the source of the DOB Image preview to the selected image
                dobImagePreview.src = e.target.result;
            };

            reader.readAsDataURL(file);
        }
    }
</script>
    </body>
</html>
